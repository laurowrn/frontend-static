...
import { GetEventsByUserResponse } from "@/infrastructure/EventGateway";
import { useSession } from "@/context/AuthContext";

export const useUserEventsData = () => {
  const { eventGateway } = useGateway();
  const [events, setEvents] = useState<GetEventsByUserResponse[] | null>(null);
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const collectedEvents = await eventGateway.getEventsByUser(
          session || ""
        );
          ...
          const fetchedAddresses = await Promise.all(
            collectedEvents.map(async (event) => {
              if (event.event.latitude && event.event.longitude) {
                ...
                if (result?.address) {
                  return `${event.event.addressName}, ${
                    result.address.city
                      ? result.address.city
                      : result.address.town
